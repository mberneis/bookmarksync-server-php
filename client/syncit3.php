...
{
	// sheesh!
//	$bm_row = my_stripslashes($bm_row);		// this was the culprit!!!!!!! duh!!
	$bm_row = trim($bm_row);
	...
	if ($cmd == "A"){
		...
		$res = my_mysql_query("insert into link (expiration,person_id,access,path,book_id) values (NULL," . $ID . ",now(),'" . my_addslashes($path) . "'," .$bid . ")");
		// Bookmark exists already - but expired?: Unexpire!
		if (!$res)
			my_mysql_query("update link set expiration = NULL,book_id = " . $bid . " where person_id = " . $ID . " and path = '" . my_addslashes($path) . "'");
	...
	// make directory
		...
		$res = my_mysql_query("insert into link (expiration,person_id,access,path) values (NULL," . $ID . ",now(),'" . my_addslashes($path) . "')");
		if (!$res)
			my_mysql_query("update link set expiration = NULL, book_id = NULL where person_id = " . $ID . " and path = '" . my_addslashes($path) . "'");
	}

	// remove directory
	else if ($cmd == "R")
		my_mysql_query("update link set expiration = now() where path = '" . my_addslashes($path) . "' and person_id = " . $ID);
