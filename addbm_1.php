...
<?php
	...
	if ($path != "")
	{
		...
		if (mysql_affected_rows() == 0)
			mysql_query("insert into link (expiration,person_id,book_id,access,path) values (null," . $AID . "," . $book_id . ",now()," . my_addslashes($path) . ")");
...
<?php
		...
		while($data = mysql_fetch_assoc($res)){
			...
			while(strtoupper(substr($p,0,strlen($prefix))) != strtoupper($prefix)){
			...
			do {
				$i = strpos(strlen($prefix)+1,$p,$delim);
				if ($i === false || $i == 0)
					break;
			
				// go forward...
				$f = substr($p,strlen($prefix),$i-strlen($prefix)-1);
				$spc = "";
				for ($s=1; $s<=$level; $s++)
					$spc .= "&nbsp;-&nbsp; ";
				
				$dbpath = $prefix . $f;
				$dbpath = substr($dbpath,strlen($dbpath)-(strlen($dbpath)-1));
				if (strlen($prefix) > 1){
					$froot = substr($prefix,strlen($prefix)-(strlen($prefix)-1));
					$froot = substr($froot,0,strlen($froot)-1);
				}

				echo "<option value='" . $dbpath . "'>" . $spc . $f . "</option>\r\n";// Directory
				$level++;
				$prefix = substr($p,0,$i);
			} while (true);
		}
