...
$sql = "select book_id,path from link where person_id=" . $ID . " and book_id is not null and expiration is null order by book_id";
...
if (isset($_POST['token']) && $_POST['token'] != ""){
	...
	if (intval($token) == intval($_POST["token"])){
		...
		$res = mysql_query($sql);
		if (!$res)
			dberror("Cannot retrieve data in duplicate.php");
	
		$cnt = 0;
		$strtemp = "";
	
		while($data = mysql_fetch_assoc($res))
			$strtemp .= my_addslashes($data["path"]) . "\r";
		...
		do {
			...
			$strtoken = substr($strtemp,0,$i);
			$strtemp = substr($strtemp,strlen($strtemp)-(strlen($strtemp)-$i));
	
			if (strpos($deldup," " . $cnt . ",")){
				echo "update link set expiration = now() where person_id=" . $ID . " and path='" . $strtoken . "'";
...
<?php
	...
	while($data = mysql_fetch_assoc($res)){
		...
		$b = $data["book_id"];
		$delstr = "<td>" . $p1 . "</td><td><a target=_blank href='showbm.php?bid=" . $b . "'>" . $p2 . "</a></td><td align=right><input name=deldup value = " . $cnt . " type=checkbox " . $chk . "></td></tr>";
		if ($b != $lastb){
			$lastb = $b;
			$firststr = $delstr;
		}
